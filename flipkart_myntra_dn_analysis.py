...
SHEET_CONFIGS = [
    {
        'sheet_id': "16cu-dfOJV4cg-H9SJSJ3_E6ru5nY7n09c_oRtpj8kQQ",
        'worksheets': [
            "FK + Myntra Q2 DN Data 26 Nov",  # Latest - appears first
            "FK + Myntra Q2 DN Data 14 Nov",
            "FK + Myntra Q2 DN Data 03 Nov"   # Oldest - appears last
        ]
    },
    {
        'sheet_id': "1_hdBboCkXhJV5Vbxgdv1NnFgWm72_ErqxXMyueTYCZI",
        'worksheets': [
            "Prexo 24 Nov DN Data"
        ]
    },
    {
        'sheet_id': "1g0rHm_QT3l4dv_bP5A_l5sjZ2wxSDhai_Rx2yaorzG8",
        'worksheets': [
            "Ext APST 20 Nov",
            "Ext RTO 21 Nov",
            "Ext RVP 21 Nov"
...
GOOGLE_SHEET_ID = "16cu-dfOJV4cg-H9SJSJ3_E6ru5nY7n09c_oRtpj8kQQ"
WORKSHEET_NAMES = [
    "FK + Myntra Q2 DN Data 26 Nov",
    "FK + Myntra Q2 DN Data 14 Nov",
    "FK + Myntra Q2 DN Data 03 Nov"
]
...
class Q2DNAnalyzer:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Handle different column names for Tracking ID (TrackingID, TID)
        if 'TrackingID' not in df_clean.columns and 'TID' in df_clean.columns:
            df_clean['TrackingID'] = df_clean['TID']
        
        # Handle different column names for Hub Name (Hub Name, Hub Name as per ERP)
        if 'Hub Name' not in df_clean.columns and 'Hub Name as per ERP' in df_clean.columns:
            df_clean['Hub Name'] = df_clean['Hub Name as per ERP']
        
        # Clean text columns
        text_columns = ['Loss Type', 'Reject Reason', 'Hub Name', 'Region', 'LSN State', 
                        'Pln_Owner', 'TrackingID', 'TID', 'Ops Remarks', 'Image Proof']
    ...
    def run(self):
        ...
        try:
            ...
            # Send email with TOP 100 high value shipments
            logging.info(f"\nSending email with TOP 100 high value shipments...")
            try:
                if combined_missing_ops is not None and not combined_missing_ops.empty:
                    ...
                    self.send_top50_email(combined_missing_ops)
                else:
                    logging.warning("No TOP 100 data found - skipping email")
            except Exception as e:
        ...
        try:
            ...
            for clm_name, clm_data in clm_groups:
                try:
                    ...
                    msg['Subject'] = f"Debit Note Alert - Missing Proof - High Value - {clm_name} Hubs - {datetime.now().strftime('%d %b %Y %H:%M')}"
                    ...
                    <body>
                        ...
                        <div class="clm-info">
                            ...
                            <ul style="margin-top: 5px;">
                                <li><a href="https://docs.google.com/spreadsheets/d/16cu-dfOJV4cg-H9SJSJ3_E6ru5nY7n09c_oRtpj8kQQ/edit" style="color: #1a73e8; text-decoration: none;">FK + Myntra Q2 DN Data</a></li>
                                <li><a href="https://docs.google.com/spreadsheets/d/1_hdBboCkXhJV5Vbxgdv1NnFgWm72_ErqxXMyueTYCZI/edit" style="color: #1a73e8; text-decoration: none;">Prexo DN Data</a></li>
                                <li><a href="https://docs.google.com/spreadsheets/d/1g0rHm_QT3l4dv_bP5A_l5sjZ2wxSDhai_Rx2yaorzG8/edit" style="color: #1a73e8; text-decoration: none;">External APST/RTO/RVP Data</a></li>
                            </ul>
                        </div>
    ...
    def send_top50_email(self, top50_df: pd.DataFrame):
        """Send email with TOP 100 high value shipments missing Ops Remarks"""
        try:
            # Create message
            msg = MIMEMultipart()
            msg['From'] = EMAIL_CONFIG['sender_email']
            ...
            msg['Subject'] = f"Debit Note Alert - Missing Proof - High Value - {datetime.now().strftime('%d %b %Y %H:%M')}"
            ...
            <body>
                <div class="header">
                    <h2>Q2 DN - TOP 100 High Value Shipments Missing Ops Remarks</h2>
                    <p><strong>Generated:</strong> {datetime.now().strftime('%d %B %Y at %H:%M:%S')}</p>
                </div>
                ...
                <div class="warning">
                    <h3>TOP 100 High Value Shipments - Missing Ops Remarks</h3>
                    <table>
                        <tr>
                            <th>S.No</th>
                            <th>Tracking ID</th>
